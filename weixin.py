...
class IndexHandler(BaseHandler):
    ...
    def post(self, *args, **kwargs):
        ...
        elif content in cityconvert:
            url = 'http://www.weather.com.cn/data/sk/%s.html' % cityslist[content]

            html = requests.get(url).content
            html = json.loads(html)
            
            weatherinfo = ""
            for k, v in html["weatherinfo"].items():
                s = "%s: %s" % (k, v)
                weatherinfo += s + " "
            content = weatherinfo.encode('utf-8')
